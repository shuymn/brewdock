...
# ADR 0003: Prefer manifest-targeted relocation before clonefile-first copy
...
Use manifest-targeted relocation as the next implementation choice. Defer `clonefile`-first copy.

- The benchmark appendix generated by `./tests/vm-pipeline-baseline.sh --output docs/pipeline-baseline.md` now replays both candidates on representative `jq` and `wget` extracted bottles captured from the VM store.
...
The next optimization Theme should derive a relocation manifest from extracted bottle payloads and use it to avoid full-tree placeholder scans during materialize/relocate.
