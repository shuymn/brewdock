...
Bottle and source execution are staged behind an explicit execution plan so bounded `network acquire`, bounded pre-finalize local prepare, checksum-complete blob publication, `defer-to-finalize` source admission, and Homebrew-visible finalize remain separate contracts instead of ad-hoc control flow.
