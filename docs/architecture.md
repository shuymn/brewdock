...
Install orchestration is stage-driven via an explicit execution plan. Bottle installs flow through `network acquire -> local prepare -> finalize`, where network acquire and local prepare both use explicit bounded concurrency, blob/store publication happens only after checksum-complete success, and finalize remains the only Homebrew-visible mutation boundary. Source installs may share planning/prefetch machinery, but source build execution remains on the finalize side until a later theme closes the shared staged executor contract.
