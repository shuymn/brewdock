...
- Use [`tests/vm-pipeline-baseline.sh`](../tests/vm-pipeline-baseline.sh) to capture brewdock-only `update` / `install` / `upgrade --dry-run` baselines with tracing-derived phase breakdowns inside a disposable Tart macOS VM. The generated Markdown also appends the copy-strategy spike appendix for representative extracted `jq` / `wget` bottles. When variance is high, pass `--runs 3` (or at most `--runs 5`) so the generated Markdown aggregates scenario wall and per-phase `wall` / `busy` / `idle` / `child` timings with median plus mean/min/max spread: `./tests/vm-pipeline-baseline.sh [--keep] [--runs 3] [--output docs/pipeline-baseline.md]`.
...
- The benchmark scripts use the same VM prerequisites as the smoke test. `vm-benchmark.sh` also requires outbound network access inside the VM for the Homebrew / zerobrew / nanobrew installers.
