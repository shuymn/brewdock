...
- Use [`tests/vm-pipeline-baseline.sh`](../tests/vm-pipeline-baseline.sh) to capture brewdock-only `update` / `install` / `upgrade --dry-run` baselines with tracing-derived phase breakdowns inside a disposable Tart macOS VM: `./tests/vm-pipeline-baseline.sh [--keep] [--output docs/pipeline-baseline.md]`.
