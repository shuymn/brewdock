...
- Use [`tests/vm-pipeline-baseline.sh`](../tests/vm-pipeline-baseline.sh) for brewdock-only phase baselines on `bd update`, `bd install tree`, `bd install jq wget`, and `bd upgrade --dry-run jq`.
